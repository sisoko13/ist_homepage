...
export default function PublicationsSectionISR({ initialPublications }: PublicationsSectionProps) {
  ...
  const refreshPublications = async () => {
    try {
      ...
      if (response.ok) {
        const latestPublications = await response.json()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await refreshPublications() // 최신 데이터로 새로고침
      } else {
    ...
    <section className="py-16 px-4 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-dark-blue-800 mb-4">Publications</h1>
